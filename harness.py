...
def safe_restore():
    ...
    os.system(f"git restore --staged verif/")
...
class Harness():
    ...
    def format_compose(self, composer : str = "docker-compose.yml"):
            ...
            cwd      = os.getcwd()
            volumes  = [ f'-v "{cwd}/{vol}:/code/{vol}"' for vol in ["docs", "rtl", "verif", "rundir"] ]
            ...
            cmd = f"docker compose -f {composer} run {volumes} --rm --build"
            print(f"{cmd}")
    ...
    def evaluate(self, id : int, checkout : bool = True):
        ...
        if hash != None and checkout:
            ...
            print(f"Using available files in rtl, docs and verif folders.")
        else:
